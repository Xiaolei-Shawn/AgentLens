...
function NodeCard({
  ...
  return (
    ...
    >
      <span className="node-view__card-icon" data-kind={kind} aria-hidden />
...
export function NodeView({ session, currentIndex, onSeek, onOpenInFlowView }: NodeViewProps) {
  ...
  const updateStrip = () => {
    ...
    for (let i = 0; i < events.length; i++) {
      ...
      const cardW = i === currentIndex ? CENTER_CARD_WIDTH : SIDE_CARD_WIDTH;
      totalWidth += cardW + (i < events.length - 1 ? CONNECTOR_WIDTH : 0);
    }
    const centerLeft = positions[currentIndex];
    const centerCardW = CENTER_CARD_WIDTH;
  ...
  return (
    <div className="node-view">
      ...
      {onOpenInFlowView && (
        <div className="node-view__actions">
          <button
            type="button"
